...
for vm in "${VMS[@]}"; do
    ...
    ssh $VM_SSH_OPTIONS -t "$vm" "
        ...
        sudo ~/install-etcd.sh $VMS $ROOT_CA_FILE_NAME $IFNAME
        sudo chmod +x ~/install-kubernetes-binaries-on-control-plane.sh
        sudo ~/install-kubernetes-binaries-on-control-plane.sh $VMS $ROOT_CA_FILE_NAME $ROOT_CA_KEY_FILE_NAME $SERVICE_CLUSTER_IP_RANGE $SERVICE_NODE_PORT_RANGE $CLUSTER_CIDR $CLUSTER_NAME $IFNAME"
    ((idx=idx+1))
done
